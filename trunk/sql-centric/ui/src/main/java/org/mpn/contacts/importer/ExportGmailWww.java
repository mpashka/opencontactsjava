...
//            log.info("Export : " + entry.getTitle().getPlainText());
...
//            ContactEntry addedContact = myService.insert(feedUrl, entry);
            if (legacyEmails.isEmpty()) continue;
            entry.getEmailAddresses().removeAll(legacyEmails);
            if (entry.getEmailAddresses().isEmpty()) entry.delete();
            else entry.update();
        }
    }
...
//            log.debug("   New phones : " + phoneNumbers);
            entry.getPhoneNumbers().clear();
            entry.getPhoneNumbers().addAll(phoneNumbers);
        }

//        entry.setExtension(new Con);
//        name.toString(), email
//                , "Personal", phones, emails, ims
